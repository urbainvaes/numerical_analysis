...
To install a package from the Julia REPL (Read Evaluate Print Loop, also more simply called the Julia shell),
...
A function \julia{fun} defined by a package \julia{pack} can be accessed as \julia{pack.fun}.
For example, to plot the cosine function from the Julia shell or in a script, write
\begin{minted}{julia}
    import Plots
    Plots.plot(cos)
\end{minted}
...
\begin{task}
    ...
    \]
    Write a function \julia{euler_constant(N)} that returns an approximation of the Euler constant by truncating the sum after $N$ terms.
\end{task}
...
\begin{task}
    ...
    \end{itemize}
    This suggests a recursive procedure for solving the problem, known as the Frame-Steward algorithm.
