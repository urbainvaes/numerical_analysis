...
The \julia{return} keyword can used for returning a value to the function caller.
...
Here the row index \julia{:} means ``select all line'' and the column index~\julia{2:3} means ``select columns 2 and 3''.
...
\begin{remark}
    ...
    That is, \julia{x = [1, 2, 3]} and \julia{x = [1; 2; 3]} both produce the same result:
    a \julia{Vector} object with elements $(1, 2, 3)$.
    We emphasize that \julia{x} is \emph{not} a \julia{Matrix} object.
    In contrast, the expression \julia{x = [1 2 3]} produces a matrix with one row and three columns.
\end{remark}
