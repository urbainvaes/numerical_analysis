...
% \begin{remark}
%     [Short-hand \julia{for} notation]
%     There is also a convenient short-hand notation for nested \julia{for} loops,
...
and \julia{M'} is the transpose of $M$.
...
\begin{itemize}
    \item \julia{[i^2 for i in 1:5]} creates the vector $(1, 4, 9, 16, 25]$.
...
which is available in a part of the Julia manual dedicated to multi-dimensional array,
...
Broadcasting, however, is much more flexible,
in that it can take arrays with different sizes.
For instance, \julia{broadcast(gcd, 24, [10, 20, 30])} returns an array of size 3 with the greatest common divisors of $(24, 10)$, $(24, 20)$ and $(24, 30)$.
Likewise, the instruction \julia{broadcast(+, 1, [1, 2, 3])} returns \julia{[2, 3, 4]}.
...
Likewise, the expression \julia{1 .+ [1, 2, 3]} returns the array \julia{[2, 3, 4]}.
