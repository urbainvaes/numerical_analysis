...
However, it is possible to derive an error estimate with a dependence on the condition number of $\kappa_2(\mat A)$ from this equation.
\begin{theorem}
    It holds that
    \[
        \forall k \geq 0, \qquad
...
\begin{equation}
    ...
    1 + \lambda q_k(\lambda) = \frac{T_{k+1}\left(\frac{\lambda_n + \lambda_1 - \lambda}{\lambda_n - \lambda_1}\right)}{T_{k+1}\left(\frac{\lambda_n + \lambda_1}{\lambda_n - \lambda_1}\right)},
\end{equation}
...
\begin{equation}
    ...
    \qquad \mu_j = \lambda_n + \lambda_1 - (\lambda_n - \lambda_1) \cos \left( j \pi \right).
\end{equation}
...
\[
    ...
    \end{cases}
\]
But this implies, by the intermediate value theorem,
that $t_{k+1}$ has $k+1$ roots in the interval $[\lambda_1, \lambda_n]$,
so $k+2$ roots in total,
which is impossible for a nonzero polynomial of degree $k+1$.
\end{proof}
