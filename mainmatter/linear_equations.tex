...
We present here
a criterion that can be employed in order to establish the convergence of the Gauss--Seidel and relaxation method.
\begin{proposition}
...
\begin{proof}
    ...
    \]
    Showing that this indeed defines a vector norm is the goal of~\eqref{exercise:linear_norm_induced_A}.
...
It is possible to prove its convergence for strictly diagonally dominant by rows or columns.
...
\]
\begin{proposition}
    Assume that $\mat A$ is strictly diagonally dominant (by row or by columns or both).
    Then $\rho(\mat M^{-1} \mat N) < 1$ for the Gauss--Seidel splitting.
\end{proposition}
\begin{proof}
    ...
    \]
    If $\abs{\lambda} > 1$,
    then the matrix $\mat N - \lambda \mat M$ is diagonally dominant and thus invertible.
    Therefore $\vect v = 0$, but this is a contradiction because $\vect v$ is vector of of unit norm.
    Consequently, all the eigenvalues bounded from above strictly by 1 in modulus.
\end{proof}
...
%     \left\{
%        \begin{aligned}
%         & a_{11} \Bigl(\omega^{-1} x^{(\textcolor{darkblue}{k+1})}_1 + (1 - \omega^{-1}) x^{(k)}_1 \Bigr) + a_{12} x^{(k)}_2 + \dotsb + a_{1n} x^{(k)}_n = b_1 \\
%         & a_{21} x^{(\textcolor{darkblue}{k+1})}_1 + a_{22} \Bigl(\omega^{-1} x^{(\textcolor{darkblue}{k+1})}_2 + (1 - \omega^{-1}) x^{(k)}_2 \Bigr)  + \dotsb + a_{2n} x^{(k)}_n = b_2 \\
%         & \vdots \\
%         & a_{n1} x^{(\textcolor{darkblue}{k+1})}_1 + a_{n2} x^{(\textcolor{darkblue}{k+1})}_2 + \dotsb + a_{nn} \Bigl(\omega^{-1} x^{(\textcolor{darkblue}{k+1})}_n + (1 - \omega^{-1}) x^{(k)}_n \Bigr) = b_n.
%        \end{aligned}
%    \right .
% \end{equation*}
...
\begin{exercise}
    ...
    Show that if $\mat A$ is diagonally dominant by rows or columns,
...
\begin{compexercise}
    Implement the Jacobi and Gauss--Seidel methods for finding a solution to the following linear system on $\real^n$.
    \[
        \frac{1}{h^2}
        \begin{pmatrix}
            2 & -1 \\
        ...
        h = \frac{1}{n}.
    \]
    Plot the norm of the residue as a function of the iteration index in both cases.
\end{compexercise}
