...
\end{proof}
A byproduct of~\cref{lemma:linear_inverse_product_gaussian_transformations} is that all the diagonal entries of the lower triangular matrix $\mat L$ are equal to 1;
...
We emphasize, however, that the sparsity structure within the band of $\mat A$ may be destroyed in $\mat L$ and $\mat U$;
...
which is much better than the general scaling as $\mathcal O(n^3)$ if $k \ll n$.
...
it is natural to wonder whether the bandwidth of a matrix $\mat A$ can be reduced via permutations.
...
permutation matrix~$\mat P$ in the particular case of a \emph{symmetric} matrix.
It is available in most programming languages and based on the fact that,
...
it returns $\bigl(\sigma^{-1}(1), \dots, \sigma^{-1}(n)\bigr)$.
...
and initialize $R = (i)$. 
Then the following steps are repeated until $R$ is of size $n$:
\begin{itemize}
    \item 
        Define $A_i$ as the set of the nodes adjacent to a node in $R$ but not themselves in $R$;

    \item 
        Sort the nodes in $A_i$ according to the following rules:
        a node $i$ comes before $j$ if $i$ is connected to a node in $R$ that comes before all the nodes of $R$ to which $j$ is connected.
        As a tiebreak, 
        give priority to nodes with higher degree.
...
\begin{figure}
    ...
    \caption{%
        ...
        The first vertex was chosen randomly since all the nodes have the same degree.
    }
\end{figure}
