...
\begin{proof}
    ...
    If $\rho(\mat M^{-1} \mat N) \geq 1$ and $\vect e^{(0)}$ is proportional to the eigenvector corresponding to the eigenvalue with largest modulus,
    then the right-hand side of this equation does not converge to 0 in the limit as $k \to 0$,
    implying that the condition $\rho(\mat M^{-1} \mat N) < 1 $ is necessary to ensure convergence for all initial guess $\vect x^{(0)}$.
    ...
    \end{equation}
    Therefore, we deduce that $\norm[bigl]{(\mat M^{-1} \mat N)^{k}} \to 0$,
    and so $\vect e^{(k)} \to \vect 0$, if $\rho(\mat M^{-1} \mat N) < 1$.
    ...
    \]
    Rearranging this inequality gives the second part of the statement.
\end{proof}
...
\begin{exercise}
    ...
    \begin{enumerate}
        \item
            ...
            where $\vect x_*$ is the exact solution to~\eqref{eq:linear_system_midterm}.
            Prove that
            \[
