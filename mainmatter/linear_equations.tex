...
The idea of partial pivoting is to rearrange the rows at each iteration of the Gaussian elimination in such a manner that
...
\begin{proposition}
    ...
    In addition, for any $\varepsilon > 0$ there exists $N > 0$ such that
    \[
        \forall n \geq N,
        \qquad
        \norm{\vect e^{(n)}} \leq \bigl(\rho(A) + \varepsilon\bigr)^n \norm{\vect e^{(0)}}.
    \]
\end{proposition}
\begin{proof}
    ...
    \[
        \vect e^{(n+1)} = (\mat M^{-1} \mat N)^2 \vect e^{(n-1)} = \dots = (\mat M^{-1} \mat N)^{n+1} \vect e^{(0)}.
    \]
    We conclude that
    \[
        \forall n \geq 0, \qquad
        \norm{\vect e^{(n)}}
        \leq \norm[bigl]{(\mat M^{-1} \mat N)^{n}} \norm{\vect e^{(0)}}.
    \]
    By Gelfand's formula, see~\cref{proposition:matrices_gelfands},
    it holds that
    \[
        \lim_{n \to \infty} \norm[bigl]{(\mat M^{-1} \mat N)^{n}}^{\frac{1}{n}} = \rho(\mat M^{-1} \mat N).
    \]
    In particular, for all $\varepsilon > 0$ there is $N \in \nat$ such that
    \[
        \forall n \geq N, \qquad
        \norm[bigl]{(\mat M^{-1} \mat N)^{n}}^{\frac{1}{n}} \leq \rho(\mat M^{-1} \mat N) + \varepsilon.
    \]
...
we want this spectral radius to be as small as possible,
...
\]
We observe that the smaller the conditioning the better the asymptotic rate of convergence.
