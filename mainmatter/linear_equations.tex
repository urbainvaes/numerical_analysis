...
\begin{proof}
    ...
    \begin{align*}
        E_{k+1}
        &= 2 g\bigl( \vect e^{(k+1)} \bigr)
        = 2  g\bigl( \vect e^{(k)} - \omega_k \vect d \bigr)
        = E_k - 2 \omega_k \vect d^\t \mat A \vect e^{(k)} + \omega_k^2 \vect d^\t \mat A \vect d \\
...
as another optimization problem which is of the same but involves a matrix with a lower condition number,
...
except that the matrix is $\widetilde {\mat A} := \mat T^\t \mat A \mat T$ instead of~$\mat A$ and the vector is $\widetilde {\vect b} := \mat T^{\t} \vect b$ instead of $\vect b$.
Its minimizer is $\vect y_* = \mat T^{-1} \vect y$.
...
\end{align*}
Consequently, the convergence is faster than that of the usual steepest descent method if and only if $\kappa_2(\mat T^\t \mat A \mat T) < \kappa_2(\mat A)$.
...
Assume that $\vect d_0, \dotsc, \vect d_{n-1}$ are $n$ pairwise $\mat A$-orthogonal directions.
By \cref{exercise:linear_independence_conjugate_directions},
these vectors are linearly independent,
and so they form a basis of $\real^n$.
Consequently, for any initial guess $\vect x_0$ the exact solution~$\vect x_*$ to the linear system $\mat A \vect x = \vect b$
...
\begin{proof}
    ...
    Then preforming the $(k+1)$-th step of the algorithm gives
    \[
...
% Note that we did not need to use the definition of $\vect d$;
% equation~\eqref{eq:linear_optimal_step_size_steepest_descent} gives the step size minimizing~$f(\vect x^{(k+1)}$ in any descent direction~$\vect d$.
