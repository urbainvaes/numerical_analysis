...
Of course,
in practice the matrix multiplications in this algorithm are not implemented as actual matrix multiplications.
...
the solution to the linear system can be obtained by first using backward, and then forward substitution,
...
we presented in \cref{sub:lu_decomposition} relies on the assumption that the pivot elements $A^{(k)}_{k+1,k+1}$ are nonzero.
In this section, 
we show how this assumption can be lifted,
provided that we use a small modification, called \emph{pivoting},
to the algorithm.
...
\begin{exercise}
    ...
    Prove that the product of lower triangular matrices is lower triangular.
\end{exercise}
