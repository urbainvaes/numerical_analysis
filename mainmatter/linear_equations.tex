...
In this section, we only discuss the partial pivoting method.
...
\end{equation}
Here $\mat P_{k+1}$ is a row permutation matrix which,
...
we note that $\mat P$ is row permutation matrix.
\begin{definition}
    \label{definition:row_permutation_matrix}%
    Let $\sigma : \{1, \dotsc, n\} \to \{1, \dotsc, n\}$ be a permutation,\
    i.e.\ a bijection on the set~$\{1, \dotsc, n\}$.
...
A permutation matrix has a single entry equal to~1 per row and per column,
and its inverse coincides with its transpose:~$\mat P^{-1} = \mat P^\t$.
Consequently, in view of~\eqref{eq:plu_decomposition}, the solution to the linear system $\mat A \vect x = \mat b$ can be obtained by solving $\mat L \mat U \vect x = \mat P^\t \vect b$,
