...
\begin{proof}
    ...
    we conclude that $\mat L$ is unique, and therefore also $\mat U = \mat L^{-1} \mat A$.
\end{proof}
...
\end{equation}
Here $\mat P_{k+1}$ is a particular type of a row permutation matrix which,
...
\end{proof}
The expression of $\mat L$ in \cref{lemma:linear_matrix_l_pivoting} suggests the iterative procedure~\cref{algo:LU_decomposition_with_partial_pivoting} for performing the factorization.
\begin{algorithm}
...
\State Assign $\mat L \gets \mat I + \begin{pmatrix} \vect c^{(1)} & \hdots & \vect c^{(n-1)} \end{pmatrix}$.
\end{algorithmic}
\end{algorithm}
...
\end{lemma}
\begin{proof}
    By the spectral theorem (see \cref{theorem:spectral_theorem} in \cref{cha:vectors_and_matrices}),
    there exists a diagonal matrix $\mat D$ and an orthogonal matrix $\mat Q$ such that
    \[
        \mat A = \mat Q \mat D \mat Q^\t.
    \]
    The diagonal entries of $\mat D$ are positive since $\mat A$ is positive definite.
    Therefore we can defined $\sqrt{\mat D}$ as
\end{proof}
