...
these diagonal entries can be omitted and the matrices $\mat L$ and $\mat U$ can be stored \emph{within} the same matrix.
...
Then the following steps are repeated until $R$ contains all the nodes:
\begin{itemize}
    \item 
...
Many of the basic iterative methods may be viewed as particular cases of a general \emph{splitting method}.
...
\end{equation}
There is the compromise between the cost of a single step and the speed convergence.
