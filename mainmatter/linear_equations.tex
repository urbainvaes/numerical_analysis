...
The idea of partial pivoting is to rearrange the lines at each iteration of the factorization in such a manner that
...
\end{equation}
where $\mat M_{k+1}$ is the Gaussian transformation matrix ensuring that
$\mat A^{(k+1)}$ is upper triangular up to column $k+1$,
and $\mat P_{k+1}$ is a row permutation matrix whose action on $\mat A^{(k)}$ leads to an interchange between row $k+1$ and another row with index $\ell \geq k+1$.
(Note that if $\ell = k+1$,
then $\mat P_{k+1}$ is the identity operator.)
...
In fact, we can restrict the row permutation matrix in~\eqref{eq:step_partial_pivoting} to one that involves only row $k+1$ and another row with index $i \geq k+1$.
The action of such a permutation matrix is to interchange rows $k+1$ and $i$,
and if $i = k+1$ then the permutation reduces to the identity operator.

The resulting matrix $\mat A^{(n-1)}$ after $n-1$ steps of $\mat L \mat U$ decomposition with partial pivoting is upper triangular and satisfies
\[
...
\]
The first factor in this decomposition of $\mat A$ is not necessarily lower triangular.
...
we note that $\mat P$ is row permutation matrix and
that the linear system $\mat A \vect x = \mat b$ can be obtained by solving $\mat L \mat U \vect x = \mat P^\t \vect b$,
the right-hand side of which can be calculated very efficiently.
\begin{lemma}
    \label{lemma:linear_matrix_l_pivoting}
    The matrix $\mat L = \mat P \mat M^{-1}$ is unit lower triangular
    with all entries bounded from above by 1.
...
\begin{proof}
    ...
    \begin{equation}
        ...
        + \dotsb
        + (\mat P_{k} \vect c^{(k-1)}) \vect e_{n-2}^\t
        + \vect c^{(k)} \vect e_{k}^\t.
    \end{equation}
    for all $k \in \{1, \dotsc, n-1 \}$.
    The statement is clear for $k = 1$,
    and we assume by induction that it is true up to~$n-1$.
    Then notice that
    \begin{align*}
        \mat P^{(k)} \bigl( \mat M^{(k)} \bigr)^{-1}
        ...
        + \dotsb
        + (\mat P_{k-1} \vect c^{(k-2)}) \vect e_{n-2}^\t
...
\end{proof}
The expression of $\mat L$ in \cref{lemma:linear_matrix_l_pivoting} suggests the following iterative procedure for performing the factorization.
\begin{algorithmic}
\State $\mat A^{(0)} \gets \mat A$, $\mat P \gets \mat I$
\For{$i \in \{1, \dotsc, n-1\}$}
    \State Find the row index $k \geq i$ such that $A^{(i-1)}_{k,i}$ is maximum in absolute value.
    \State Interchange the rows $i$ and $k$ of $\mat A^{(i-1)}$, of $\mat I$, and of $\vect c^{(1)}, \dotsc, \vect c^{(i-1)}$.
    ...
    \State $\mat A^{(i)} \gets \mat M_{i} \mat A^{(i-1)}$
\EndFor
\State $\mat U \gets \mat A^{(n-1)}$.
\State $\mat L \gets \mat I + \begin{pmatrix} \vect c^{(1)} & \hdots & \vect c^{(n-1)} \end{pmatrix}$.
\end{algorithmic}
\begin{exercise}
...
\begin{exercise}
    ...
    Implement the $\mat L \mat U$ factorization algorithm with partial pivoting.
    Compare the condition number of the matrices $\mat L$ and $\mat U$ to the case without pivoting.
