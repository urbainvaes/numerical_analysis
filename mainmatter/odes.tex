...
\begin{theorem}
    ...
    \begin{equation}
        ...
        \vecnorm{\vect x(t_n) - \vect x_n}
        \leq
        \frac{\Delta M}{2} \left( \frac{\left( \frac{1}{1 - \Delta L} \right)^n - (1 - \Delta L)}{L} \right).
    \end{equation}
\end{theorem}
\begin{proof}
...
each amplified by a factor which decreases as the number of iterations left to reach the final time decreases.
...
\begin{equation}
    ...
    \qquad \tau \in (t, t + \Delta).
\end{equation}
If $\vect x\colon [0, T] \to \real$ is a smooth solution to~\eqref{eq:ode},
then the time derivatives of~$\vect x$ can be obtained by differentiation of~\eqref{eq:ode}:
\[
...
\begin{align}
    ...
    \nonumber
    &= \frac{1}{\Delta} \Bigl( \vect f \bigl( t_{n+1}, \vect x^{t_n, \vect x_n}(t_{n+1}) \bigr) - \vect f \bigl( t_{n}, \vect x_n \bigr) \Bigr) \\
    \label{eq:approx_heun}
    &\approx \frac{1}{\Delta} \Bigl( \vect f \bigl( t_{n+1}, \vect x_n + \Delta \vect f(t_n, \vect x_n) \bigr) - \vect f \bigl( t_{n}, \vect x_n \bigr) \Bigr)
\end{align}
...
\[
    ...
    + \frac{\Delta}{2} \vect f \bigl( t_{n+1}, \vect x_n + \Delta \vect f(t_n, \vect x_n) \bigr).
...
and this issue is not addressed in this course.
...
The class of Runge--Kutta methods is very wide and also includes implicit schemes,
of which we give a few examples:
\begin{itemize}
    \item 
        The Crank--Nicolson method:
        \begin{equation}
            \label{eq:crank_nicolson}
            \vect x_{n+1} = \vect x_n + \frac{\Delta}{2} \bigl( f(t_n, \vect x_n) + f(t_{n+1}, \vect x_{n+1}) \bigr).
        \end{equation}
