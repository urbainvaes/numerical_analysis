...
but for simplicity we assume throughout this section that the time step is fixed.
We begin in~\cref{sub:explicit_euler} and~\cref{sub:implicit_euler} by studying the simplest one-step methods,
and then in~\cref{sub:one_step_general} we present a general approach to the analysis of one-step methods.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \Bigl( \vect x(t_{n-1}) + \Delta \, \vect f\bigl(t_{n-1}, \vect x(t_{n-1})\bigr) + \frac{\Delta^2}{2} \vect x''(\tau) \Bigr)
...
known as \emph{consistency} and \emph{stability} of the numerical method,
...
more specifically in equation~\eqref{eq:sum_local_and_accumulation},
...
For the forward Euler method we have $\Phi_{\Delta}(t, \vect x) = \vect f(t, \vect x)$,
while the backward Euler method, the function $\vect \Phi_{\Delta}$ is defined implicitly as the function which to~$(t, \vect x)$ associates the solution~$\vect \phi \in \real^n$ to the equation
\[
...
\]
Since there is a division by~$\Delta$, the local truncation error has the same physical dimension as that of $\dot{\vect x}$,
...
it bounds the norm of just one mathematical object, namely the sequence $(\vect x_n - \vect y_n)_{1\leq n \leq N}$,
...
\begin{proof}
    ...
    \[
        ...
        = (1 + \Delta L_{\Phi}) \vecnorm{ \vect x_{n-1} - \vect y_{n-1} }
...
a pillar of numerical analysis.
...
In this section, we motivate some of the other widely-used one-step methods,
...
\]
The Taylor scheme~\eqref{eq:taylor_scheme} with $p = 1$ is the forward Euler scheme.
Note that, for any~$p$,
...
\end{align*}
The local truncation error for this method scales as~$\Delta^4$.
When~$\vect f(t, \vect x) = \vect f(t)$,
this method is coincide with Simpson's formula~\eqref{eq:simpsons} for the approximation of the integral in~\eqref{eq:one_step_integral}.
The systematic derivation of Runge--Kutta methods is cumbersome,
and so we do not address it in this course.
...
we mention two common implicit methods with a better order of consistence than that of the backward Euler method.
\begin{itemize}
    \item
...
This enables to construct more economical numerical methods than one-step methods for the same order of convergence,
at the cost of a more difficult initialization.
In this section we focus on~\emph{linear} multistep methods of the form
 \begin{align}
     \nonumber
     \vect x_{n+1}  &= a_1 \vect x_n + a_2 \vect x_{n-1} + \dotsb + a_k \vect x_{n-k+1} \\
     \label{eq:linear_mulitstep}
                    &\qquad + \Delta \Bigl( b_0 \vect f(t_{n+1}, \vect x_{n+1}) + b_1 \vect f(t_{n}, \vect x_{n}) + \dotsb + b_k \vect f(t_{n-k+1}, \vect x_{n-k+1}) \Bigr).
\end{align}
This equation defines an explicit method if $b_0 = 0$,
and an implicit method if $b_0 \neq 0$.
Explicit methods of the form~\eqref{eq:linear_mulitstep} require only one additional evaluation~$f(t_{n}, \vect x_{n})$ per iteration.
...
Similarly to the setting of one-step methods,
...
\begin{align}
    \nonumber
    \Delta \vect \eta_{n+1}
        &:= \vect x(t_n + \Delta) - a_1 \vect x(t_n) - a_2 \vect x(t_n - \Delta) - \dotsb - a_k \vect x\bigl(t_n - (k-1) \Delta\bigr) \\
        \label{eq:truncation_multistep}
        & \qquad- \Delta \Bigl( b_0 \vect x'(t_n + \Delta) + b_1 \vect x'(t_n) + \dotsb + b_k \vect x'\bigl(t_n - (k-1) \Delta\bigr) \Bigr).
\end{align}
The multistep method~\eqref{eq:linear_mulitstep} is of order~$p$ if the maximum of the local truncation error
over all the discretization points scales as~$\bigo(\Delta^p)$.
The following result is useful for constructing multistep methods.
\begin{proposition}
    The linear multistep method~\eqref{eq:linear_mulitstep} is of order~$p$ if and only if
    the local truncation error is everywhere zero when the exact solution $\vect x(t)$ is of the form
    \begin{equation}
        \label{eq:polynomial_form}
        \vect x(t) = \vect e t^{q}, \qquad q \in \{1, \dotsc, p\},  \qquad \vect e \in \real^n.
    \end{equation}
\end{proposition}
\begin{proof}
    Assume that the method is of order~$p$, fix $q \in \{1, \dotsc, p\}$ and $\vect e \in \real^n$,
    and let $\vect x(t) = \vect e t^{q}$.
    Then the right-hand side of~\eqref{eq:truncation_multistep},
    viewed as a function of~$\Delta$ for fixed~$t_n$,
    is a polynomial of degree as most~$p$ which scales as~$\bigo(\Delta^{p+1})$.
    Therefore, it holds necessarily that~$\vect \eta_{n+1} = 0$.
    ...
    If~$\vect x(t)$ denote a smooth solution of~\eqref{eq:ode},
    ...
    \]
    for appropriate vectors $\vect e_i \in \real^n$.
...
\end{proof}
\begin{example}
    In the one-dimensional setting~$n = 1$,
    consider for example a multistep scheme of the form
     \[
         x_{n+1}  = a_1 x_n + a_2 x_{n-1} + b_1 \Delta f(t_{n}, x_{n}).
    \]
    We wish to find the parameters $a_1$, $a_2$ and $b_1$ so that the order of consistency is as high as possible.
    Substituting $x(t) = 1$ in formula~\eqref{eq:truncation_multistep} for the local error,
    we obtain
    \[
        \eta_{n+1} = x(t_n + \Delta) - a_1 x(t_n) - a_2 x(t_n - \Delta) - b_1 \Delta x'(t_n)
        = 1 - a_1 - a_2.
    \]
    Therefore $a_2 = (1 - a_1)$.
    Next, substituting $x(t) = t - t_n$,
    we obtain
    \[
        \eta_{n+1} = \Delta (2 - a_1 - b_1),
    \]
    which gives $b_1 = 2 - a_1$.
    Finally, substituting $x(t) = (t-t_n)^2$,
    we obtain
    \[
        \eta_{n+1} = \Delta^2 a_1.
    \]
    We conclude that the best parameters,
    leading to a local truncation error scaling as~$\bigo(\Delta^2)$, 
    are given by~$a_1 = 0$, $a_2 = 1$ and $b_1 = 2$.
    The resulting method reads
    \[
         x_{n+1} = x_{n-1} + 2 \Delta f(t_{n}, x_{n}),
    \]
    and is known as the midpoint multistep method.
\end{example}
