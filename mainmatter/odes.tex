...
We emphasize that both the forward and the backward Euler methods may be recast in the form~\eqref{eq:general_one_step}.
...
Whether a numerical method is absolutely stable depends on the parameters~$\lambda$ and $\Delta$.
\begin{example}
...
\[
    \mathcal A := \{ z \in \complex : \text{\eqref{eq:convergence_absolute_stability}  holds if $\Delta \lambda = z$} \} \subset \complex.
\]
...
\begin{itemize}
    ...
    \item
        ...
        If $f'(x_*) < 0$,
        then by a result $x_*$ is an attractor of the equation,
    ...
    \item
        ...
        At a given time~$t$,
        the solutions tend to converge if~$\partial_x f\bigl(t, x(t)\bigr) < 0$,
        and diverge if $\partial_x f\bigl(t, x(t)\bigr) > 0$.
        ...
        Although this statement is difficult to state precisely and prove generally,
...
\begin{example}
    ...
    \end{align*}
    Therefore, the numerical solutions $(x^{\varepsilon}_n)$ and $(x_n)$ tend to become closer if
    \begin{equation}
...
A differential equation of the form~\eqref{eq:ode} is called stiff if the Jacobian matrix~$\mat J_f\bigl(t, \vect x(t)\bigr)$ has at least one eigenvalue with a large negative real part in some part of the domain~$[0, T]$.
In the one-dimensional setting,
...
as is depicted in~\cref{fig:forward_euler_stiff}.
Note that the if the equation is very stiff,
that is to say if $\alpha \gg 1$,
then a very small time step is required to ensure stability.

In contrast to the backward Euler scheme,
the forward Euler scheme is stable regardless of the time step.
Since the right-hand side of~\eqref{eq:stiff_ode} is linear in~$x$,
the value of the iterate~$x_{n+1}$ can be calculated explicitly from~$x_n$:
\[
