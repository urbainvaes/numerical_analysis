...
A differential equation of the form~\eqref{eq:ode} is called stiff if the Jacobian matrix~$\mat J_f\bigl(t, \vect x(t)\bigr)$,
with respect to the variable~$\vect x$,
has at least one eigenvalue with a large negative real part in some part of the domain~$[0, T]$.
In the one-dimensional setting,
