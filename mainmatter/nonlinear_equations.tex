...
which we will then employ in the particular case where the matrix-valued function $\mat A$ is equal to~$\mat J_f(\vect x_*)$.
\begin{lemma}
    \label{lemma:lemma_newton_raphson}
    Let $\mat A\colon \real^n \to \real^{n \times n}$ denote a continuous matrix-valued function on $\real^n$ such that~$\mat A(\vect x_*)$ is nonsingular,
...
\begin{proof}
    ...
    \[
        \lim_{\norm{\vect h} \to 0} \frac{\norm[big]{\vect G(\vect x + \vect h) - \vect G(\vect x) - \bigl(\mat I - \mat A(\vect x_*)^{-1} \mat J_f(\vect x_*)\bigr) \vect h}}{\norm {\vect h}} = 0
    \]
...
\begin{proof}
    ...
    In addition, the second statement in~\cref{lemma:lemma_newton_raphson} gives that $\mat J_F(\vect x_*) = \mat I - \mat J_F(\vect x_*) \mat J_F(\vect x_*) = 0$,
...
\begin{exercise}
    ...
    Verify your finding numerically and plot,
