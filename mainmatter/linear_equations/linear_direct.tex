...
with a general invertible matrix~$\mat A \in \complex^{n \times n}$.
...
In this section,
we first discuss the existence and uniqueness of the $\mat L \mat U$ factorization of a matrix.
...
To this end, we define the principal submatrix of order $i$ of a matrix $\mat A \in \complex^{n \times n}$
as the matrix $\mat A_i = \mat A[1:i, 1:i]$, in Julia notation.

\begin{proposition}
    \label{proposition:linear_existence_lu}
    The $\mat L \mat U$ factorization of a matrix $\mat A \in \complex^{n \times n}$,
...
The action of a Gaussian transformation $\mat M_k$ left-multiplying a matrix $\mat A \in \complex^{n \times n}$ is
to replace the rows from index $k + 1$ to index $n$ by a linear combination involving themselves and the $k$-th row.
To see this, let us denote by $(\vect r^{(i)})_{1 \leq i \leq n}$ the rows of a matrix $\mat T \in \complex^{n \times n}$.
Then, we have
\[
    \mat M_k \mat T
...
\begin{proof}
    ...
    Denoting denote by $(\vect r^{(i)})_{1 \leq i \leq n}$ the rows of $\mat A^{(k)}$,
    we have
    \[
        \mat M_{k + 1} \mat A^{(k)}
        =
...
\[
    \overbrace{2\times}^{\text{\julia{-} and \julia{*}}} \underbrace{\sum_{i=1}^{n-1}}_{\text{\julia{for i in 1:n-1}}} \overbrace{(n - i)}^{\text{\julia{for r in i+1:n}}} \underbrace{(n - i + 1)}_{\text{indices \julia{[i:end]}}} \quad \mathrm{flops}
...
\begin{definition}
    ...
    \]
\end{definition}
When a row permutation $\mat P$ left-multiplies a matrix $\mat B \in \complex^{n \times n}$,
...
\subsection{Direct method for Hermitian positive definite matrices}%
\label{sub:direct_method_for_symmetric_positive_definite_matrices}
The $\mat L \mat U$ factorization with partial pivoting applies to any matrix $\mat A \in \complex^{n \times n}$ that is invertible.
If~$\mat A$ is Hermitian positive definite,
...
\begin{lemma}
    ...
    If~$\mat A$ is Hermitian positive definite,
    then there exists a lower-triangular matrix $\mat C \in \complex^{n \times n}$ such that
    \begin{equation}
...
\begin{proof}
    ...
    which are necessarily real and positive because $\mat A$ is positive definite.
...
the matrix $\mat A \in \complex^{n \times n}$ very often has a bandwidth which is small in comparison with $n$.
\begin{definition}
    The bandwidth of a matrix $\mat A \in \complex^{n \times n}$ is the smallest number $k \in \nat$ such that
...
permutation matrix~$\mat P$ in the particular case where $\mat A$ is a \emph{Hermitian} matrix.
It is based on the fact that,
to a Hermitian matrix~$\mat A$, we can associate a unique undirected graph whose adjacency matrix~$\mat A_*$ has the same sparsity structure as that of $\mat A$,
