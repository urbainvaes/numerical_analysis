...
\end{remark}
\subsection{Mean square approximation using orthogonal polynomials}
...
\]
The coefficients of the basis functions in this expansion are called \emph{Fourier coefficients}.
...
in to use trigonometric basis functions.
...
\subsection{Orthogonal polynomials and numerical integration: an introduction}
...
and since all the eigenvalues of~$\mat T$ are distinct,
we deduce that
\begin{equation}
...
and the superscript~$m$ is used to emphasize that the diagonal elements of this matrix depend on~$m$.)
...
\]
which means that the columns of~$\mat P$ are orthonormal for the inner product
\begin{align*}
    \ip{\vect x,\vect y}_{m+1} \colon& \real^{m+1} \times \real^{m+1}  \to \real; \\
                              &(\vect x, \vect y) \mapsto \vect x^\t \mat D^{-1} \vect y = \sum_{i=0}^{m} \frac{x_i y_i}{d_{ii}}.
\end{align*}
We have thus shown that,
if~$\varphi_0, \varphi_1, \varphi_2$ are a family of orthonormal polynomials,
then the finite-dimensional vectors
\[
    \vect \varphi_0 :=
    \begin{pmatrix}
        \varphi_0(r_0) \\
    ...
    \dotsc
\]
are orthonormal vectors in~$\real^{m+1}$ for the inner product~$\ip{\placeholder,\placeholder}_{m+1}$.
Consequently, it holds that
\begin{equation}
    \label{eq:orthogonality}
    \forall (p, q) \in \poly(m)^2, \qquad
    \ip{p,q} = \ip{\vect p, \vect q}_{m+1},
\end{equation}
where $\vect p$ and $\vect q$ are the vectors in~$\real^{m+1}$ containing the evaluations of~$p$ and~$q$ at $r_0, \dotsc, r_m$.
...
\begin{align*}
    ...
    = \sum_{i=0}^{m} \sum_{j=0}^{m} \alpha_i \alpha_j \ip{\vect \varphi_i, \vect \varphi_j}_{m+1} \\
    & = \ip{\alpha_0 \vect \varphi_0 + \dotsb + \alpha_m \vect \varphi_m,\beta_0 \vect \varphi_0 + \dotsb + \beta_m \vect \varphi_m}_{m+1} = \ip{\vect p,\vect q}_{m+1}.
\end{align*}
Taking~$q = 1$ in~\eqref{eq:orthogonality} and employing the definitions of~$\ip{\placeholder, \placeholder}$ and $\ip{\placeholder, \placeholder}_{m+1}$,
we have
\[
    \forall p \in \poly(m),
    \qquad \int_{a}^{b} p(x) \, w(x) \, \d x
    = \sum_{i=0}^{m} \frac{p(r_i)}{d_{ii}}.
\]
Since the left-hand side is an integral and the right-hand side is a sum,
we have just designed our first integration formula,
with the nice property that it is exact for polynomials of degree up to~$m$!
The nodes $r_0, \dotsc, r_m$, together with the weights $1/d_{00}, \dotsc, 1/d_{mm}$,
depend on the weight~$w(x)$ and on the degree~$m$.
More on this subject in~\cref{cha:quadrature}.
