...
\begin{remark}
    ...
    When multiplied by~$2^{n-1}$,
    the polynomial achieving the lower bound in~\eqref{eq:chebychev_lower_bound},
    oscillates between the values -1 and 1,
    which are respectively its minimum and maximum values over the interval~$[-1, 1]$.
    It attains the values 1 or -1 at $n+1$ points $x_0, \dotsc, x_n$,
    with $x_0 = -1$ and $x_n = 1$.
    It turns out that these properties,
    which can shown to hold a priori using Chebyshev's \emph{equioscillation theorem},
    is sufficient to derive an explicit expression of the polynomial~$T_n$,
    as we formally demonstrate hereafter.

    The points $x_2, \dotsc, x_{n-1}$
    are local extrema of~$T_n$,
    and so~$T_n'(x) = 0$ at these nodes.
    We therefore deduce that $T_n$ satisfies the differential equation
    \begin{equation}
        \label{eq:differential_equation_chebyshev}
        n^2\bigl(1 - T_n(x)^2\bigr) = T_n'(x)^2 (1 - x^2).
    \end{equation}
    ...
    \[
        \frac{T_n'(x)}{\sqrt{1 - T_n(x)^2}} = \pm \frac{n}{1 - x^2}
        \qquad \Leftrightarrow \qquad
        \frac{\d}{\d x} \arccos\bigl(T_n(x)\bigr) = \pm n \frac{\d}{\d x} \arccos(x).
    \]
    Integrating both sides and taking the cosine,
    we obtain
    \[
        T_n(x) = \cos\bigl(C + n \arccos(x)\bigr).
    \]
    Requiring that $|T_n(-1)| = 1$, we deduce $C = 0$.
\end{remark}
...
Hermite interpolation generalizes Lagrange interpolation to the case where
the values of some of the derivatives are given at the interpolation nodes.
For simplicity,
we assume in this section that only data on the first derivative at the nodes is available.
Given data~$(x_i, u_i, u_i')$ for~$i \in \{0, \dotsc, n\}$,
Hermite interpolation consists in finding a polynomial~$\widehat u$ of degree at most $2n + 1$
such that
\[
    \forall i \in \{0, \dotsc, n\}, \qquad
    \widehat u(x_i) = u(x_i), \qquad
    \widehat u'(x_i) = u'(x_i).
\]
In order to construct the interpolating polynomial,
it is useful to square the usual Lagrange polynomials:
\[
    \varphi_{i}(x)
    = \prod_{j=0, j\neq i} \left( \frac{x-x_j}{x_i-x_j} \right)^2
\]
The functions $\varphi_i$ thus defined satisfy
\[
    \varphi_i(x_j) = 1,
...
\section{Piecewise interpolation using cubic splines}
