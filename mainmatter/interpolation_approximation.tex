...
an orthogonal basis can be constructed via the Gram--Schmidt process.
...
In other words, the finite-dimensional vectors
\[
    \vect \varphi_0 :=
    \begin{pmatrix}
        \varphi_0(r_0) \\
...
\begin{align*}
    ...
    = \sum_{i=0}^{m} \sum_{j=0}^{m} \alpha_i \alpha_j \ip{\varphi_i, \varphi_j}_{m+1} \\
...
In fact, equation~\eqref{eq:integration_formula} is valid more generally than for~$p \in \poly(n)$.
