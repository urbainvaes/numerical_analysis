...
that the representation (sign, exponent and significand) of a nonzero number in a floating point format $\floating(p, E_{\min}, E_{\max})$ can be made unique.
In this section,
we discuss how these numbers are stored on the computer in practice.
As their name suggests, the \texttt{Float32} and \texttt{Float64} formats are using 32 and 64 bits of memory, respectively.
A naive encoding approach would be to store the sign, exponent and significand in full.
For the \texttt{Float32} format, 1 bit is required for the sign,
8 bits are needed to represent the 253 values of the exponent, and 24 bits are required for the significand,
