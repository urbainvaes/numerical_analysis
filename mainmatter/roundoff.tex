...
\begin{compexercise}
    [Numerical differentiation]
    Let $f(x) = \exp(x)$ and let \julia{d(δ)} be the approximation of $f'(1)$ given by
    \begin{minted}{julia}
        f(x) = exp(x)
        d(δ) = (f(1+δ) - f(1))/δ
    \end{minted}
    Plot \julia{abs(d(δ) - exp(1))} as a function of~\julia{δ} in logarithmic scale and explain the result.
\end{compexercise}
...
\begin{exercise}
    Does \julia{0.1 + 0.2 == 0.3} evaluate to \julia{true} or \julia{false}.
    Explain.
\end{exercise}
