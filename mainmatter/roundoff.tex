...
Nowadays, the vast majority of programming languages comply with the IEEE 754 standard,
...
\begin{table}[ht]
    ...
    \caption{%
        ...
        This format,
        which is available through the \julia{Float16} type in Julia,
...
\begin{example}
    In Julia,
    the machine epsilon can be obtained using the \julia{eps} function.
...
\begin{example}
    ...
    we check that the floating point literal \julia{1.0} is indeed interpreted as a double-precision number:
...
When a numerical computation unexpectedly returns \julia{Inf} or \julia{-Inf},
...
Since the overflow behavior of integer formats is not universal across programming languages,
...
\begin{exercise}
    ...
    More precisely, find $x \in \real$ such that the inequality is an equality.
\end{exercise}
...
\begin{solution}
    ...
    the boolean expression in the question is thus \julia{true}.
\end{solution}
...
\end{exercise}
\begin{solution}
    The exact binary representation of $x := \sqrt{2}$, found using the \julia{bitstring} in the \julia{Float64} format,
    is given by
    \begin{align*}
        x
...
\begin{solution}
    ...
    \]
    The number returned by the code \julia{exp(log(Float16(7)))} is
    \[
