...
Now that we know the set of values representable on a computer,
...
the process by which any infinitely precise real number is approximated by a floating point number of the type presented in the previous section.
The basic principles of rounding as specified by the IEEE 754 standard are the following:
\begin{itemize}
    \item 
...
\begin{itemize}
    \item An infinite binary expansion of the number is calculated;
    \item The 
\end{itemize}
...
\begin{table}[hb]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        & Single precision & Double precision
        \\ \hline
        Exponent bias & $127 = -E_{\min} + 1$ & $1023 = -E_{\min} + 1$
        \\ \hline
        Exponent encoding (bits) & 8 & 11
        \\ \hline
        Significand encoding (bits) & 23 & 52
        \\ \hline
    \end{tabular}
...
\Cref{sec:set_of_values} is based on the IEEE 754 standard~\cite{ieee754} and~\cite{MR2265914}.
