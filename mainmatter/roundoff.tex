...
\begin{itemize}
    ...
    \item
        ...
        we briefly present the standard integer formats and their machine representation.
\end{itemize}
...
\begin{exercise}
    Is \julia{Float32(0.1) * 10 == 1} equal to \julia{true} or \julia{false}
...
but the following discussion applies also to the double and half-precision formats,
provided that the format parameters are changed accordingly.
...
\section{What about integer formats?}%
...
we give a few orders of magnitude concerning integer formats and briefly discuss overflow issues.

Programming languages typically provide integer formats using 16, 32 and 64 bits.
...
and the remaining bits store the binary expansion of the integer.
This means, for example, that the \julia{Int64} format enables to store integers from $-N$ to $N = 2^{63} - 1$.
...
Since the overflow behavior is not universal across programming languages,
a detailed discussion is of little interest.
We only mention that Julia uses a \emph{wraparound} behavior,
where $N + 1$ silently returns $N$ and, 
conversely, $-N - 1$ gives $N$;
the integers loop back. 
This can lead to unexpected results,
such as \julia{2^64} returning 0.
