...
it cannot be expected that~$\mat X_k$ converges to $\mat V$,
...
\begin{theorem}
    [Convergence of the subspace iteration~\moreinfo]
...
This method, which is based on the Gram--Schmidt process,
is presented in~\cref{algo:arnoldi_method}.
\begin{algorithm}
\caption{Arnoldi iteration for constructing an orthonormal basis of $\mathcal K_p(\mat A, \vect q_1)$}%
\label{algo:arnoldi_method}%
\begin{algorithmic}
\State Choose $\vect q_1$ with unit norm.
\For{$i \in \{2, \dotsc p\}$}
    \State $\vect q_{i} \gets \mat A \vect q_{i-1}$
    \For{$j \in \{1, \dotsc, i-1\}$}
        \State $h_{j,i-1} \gets \vect q_j^* \vect q_i$
        \State $\vect q_i \gets \vect q_i - h_{j,i-1} \vect q_j$
    \EndFor
    \State $h_{i,i-1} \gets \norm{\vect q_i}$
    \State $\vect q_i \gets \vect q_i / h_{i,i-1}$
\EndFor
\end{algorithmic}
\end{algorithm}

By construction,
it is clear that the vectors $\{\vect q_1, \dotsc, \vect q_p\}$ at the end of the algorithm are orthonormal.
The scalar coefficients $h_{j,i-1}$ can be collected in a matrix
\[
    \mat H
    = 
    \begin{pmatrix}
...
% by extending the search subspace to $\{\vect x_0  \}$
...
\begin{exercise}
    ...
    \begin{itemize}
        ...
        \item
            Show that $\mat A^\t \mat O^{-1}$ is a left-stochastic matrix,
