...
Before we present the subspace iteration,
...
\begin{proof}
    ...
    Then, projecting both sides of the second equation onto $\Span \{ \vect q_1 \}^\perp$,
    we obtain
    \[
        \vect x_2 - (\vect q_1 \vect q_1^*) \vect x_2 = \alpha^2_2 \vect q_2,
    \]
    which enables to calculate $\vect q_2$ and $\alpha^2_2$.
...
would be obtained by applying the Gram--Schmidt method to the columns of~$\mat X$.
...
the subspace iteration reduces to the power iteration.
\begin{algorithm}
...
\For{$i \in \{1, 2, \dotsc\}$}
    \State $\mat Y \gets \mat A \mat X$
    \State Factorize $\mat Y = \mat Q \mat R$.
...
Mathematically, it would be equivalent to perform the $\mat Q \mat R$ decomposition only once as a final step,
...
\begin{theorem}
    ...
    Assume that $\mat A \in \complex^{n \times n}$ is diagonalizable,
