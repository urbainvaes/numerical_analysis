...
\begin{lemma}
    [Continuity of the reduced QR decomposition]
...
\begin{proof}
    ...
    \end{align*}
    It remains to show that the convergence $\vect q_i^{(k)} {\vect q_i^{(k)}}^* \to \vect v_i \vect v_i^*$ implies the desired essential convergence.
...
\end{proof}
\begin{remark}
    \Cref{proposition:invariant_subpsace_ritz} can be proved more directly from~\eqref{eq:equivalent_rayleigh_ritz} by noting that,
    if~$\mathcal U$ is an invariant subspace of~$\mat A$,
    then this equation implies that $\mat A \vect v - \lambda \vect v$ belongs to both $\mathcal U$ and its orthogonal complement $\mathcal U^\perp$,
    and so this vector is $\vect 0$.
\end{remark}
