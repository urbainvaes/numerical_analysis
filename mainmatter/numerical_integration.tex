...
\]
Although in practice we calculate the values taken $J, J_1, J_2, \dotsc$ at specific values,
these are in fact functions of~$h$.
In \cref{fig:richardson},
we plot these functions in the case where~$J(h) = 1 + \sin(h)$.
It appears clearly from the figure that,
for sufficiently small~$h$,
$J_3(h)$ provides the most precise approximation of $J(0) = 1$.
Constructing the functions in Julia can be achieved in just a few lines.
\begin{minted}{julia}
J(h) = 1 + sin(h)
J₁(h) = 2J(h) - J(2h)
J₂(h) = (4J₁(h) - J₁(2h))/3
...
In this case, the Richardson extrapolation procedure can be slightly modified to produce successively approximations with errors scaling as~$\mathcal O(h^4)$,
then $\mathcal O(h^6)$, then $\mathcal O(h^8)$.
...
Romberg's integration method simply consists of applying Richardson's extrapolation to the function
\[
...
that~$J(h)$ admits an expansion of the form
\[
    J(h) = I + \alpha_1 h^2 + \alpha_2 h^4 + \cdots + \alpha_k h^k + \mathcal O(h^{k+1}).
\]
Richardson's extrapolation~\eqref{eq:generalized_richardson} can therefore be employed in order to compute approximations of increasing accuracy.
...
This quadrature rule can be obtained more constructively from orthogonal polynomials,
...
and so roundoff errors are much less bothersome than with the Newton--Cotes method.
...
\begin{exercise}
    ...
    if $g\colon [a, b] \to \real$ is non-negative integrable function
    and $f\colon [a, b] \to \real$,
...
\end{exercise}
\begin{remark}
    One may convince oneself of~\eqref{eq:mean_value} is to rewrite this equation as
    \[
        \frac{\int_{a}^{b} f(x) g(x) \, \d x}{\int_{a}^{b} g(x) \, \d x} = f(c).
    \]
    The left-hand side is simply the average of $f(x)$ with respect to the probability measure 
