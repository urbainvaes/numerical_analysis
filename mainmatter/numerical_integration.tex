...
\end{itemize}
In theory,
this approach can be employed in order to construct integration rules of arbitrary high degree of precision.
In practice, however, the weights become more and more unbalanced,
...
In addition, in cases where the interpolating polynomial does not converge to~$u(x)$,
for example if~$u(x)$ is Runge's function,
the approximate integral may not even converge to its exact in the limit as~$m \to \infty$ in exact arithmetic!
...
In essence, this consists of partitioning the integration interval in a number of subintervals,
and using one of the rules presented in~\eqref{sec:newton_cotes} within each subinterval.
