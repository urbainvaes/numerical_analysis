...
The total number of degrees of freedom for a general integration rule of the form~\eqref{eq:deterministic_integration} is $2m + 2$ which,
in principle, should enable to construct an integration rule with a degree of precision equal to~$2m+1$.
...
equal to $2m +1$ is that it integrates exactly all the monomials of degree $0$ to~$2m+1$.
...
\begin{align*}
    \widehat I\left(\alpha_0 + \alpha_1 x + \dotsb + \alpha_{2m+1} x^{2m+1}\right)
    &= \alpha_0 \widehat I(1) + \alpha_1 \widehat I(x) + \dotsb + \alpha_{2m+1} \widehat I\left(x^{2m+1}\right) \\
    &= \alpha_0 I(1) + \alpha_1 I(x) + \dotsb + \alpha_{2m+1} I\left(x^{2m+1}\right) \\
    &= I\left(\alpha_0 + \alpha_1 x + \dotsb + \alpha_{2m+1} x^{2m+1}\right),
\end{align*}
...
we can therefore solve the following nonlinear system of $2m+2$ equations with $2m+2$ unknowns:
\begin{align}
    \label{eq:system_gauss_legendre}
    \sum_{i=0}^{m} w_i x_i^{d} &= \int_{-1}^{1} x^d \, \d x, \qquad d = 0, \dotsc, 2m+1.
\end{align}
...
Specifically, we prove the integration nodes are given by the roots of a Legendre polynomial,
which opens the door to a practical approach for calculating them.
\begin{theorem}
    ...
    The nodes $(x_i)_{i \in \{0,\dotsc m\}}$ are the roots of $L_{m+1}$
...
\end{theorem}
\begin{proof}
    We show first that the solution to~\eqref{eq:system_gauss_legendre} exists and then that it is unique.
    \paragraph{Existence.}
    We begin by showing that,
    if $(x_i)_{i \in \{0,\dotsc m\}}$ are the roots of~$L_{m+1}$ and the weights are defined from~\eqref{eq:weights_gauss},
    ...
    \begin{equation}
        ...
        \int_{-1}^{1} p(x) \, \d x = \sum_{i=0}^{m} w_i p(x_i).
    \end{equation}
    Take $p \in \poly(2m + 1)$ and let $q \in \poly(m)$ and $r \in \poly(m)$ be the polynomials such that
    \[
        p(x) = q(x) L_{m+1}(x) + r(x).
    \]
    Since $L_{m+1}$ is orthogonal to any polynomial in~$\poly(m)$
    and the nodes $(x_i)_{i \in \{0,\dotsc m\}}$ are the roots of~$L_{m+1}$,
    it holds that
    \begin{equation}
        \label{eq:intermediate_gauss}
        \int_{-1}^{1} p(x) \, \d x
        - \sum_{i=0}^{m} w_i p(x_i)
        = \int_{-1}^{1} r(x) \, \d x
        - \sum_{i=0}^{m} w_i r(x_i).
    \end{equation}
    Given that $r \in \poly(m)$,
    $r$ must coincides with its polynomial interpolation at the points $x_0, \dotsc x_m$.
    Therefore,
    \[
        r = r(x_0) \ell_0 + \dotsb + r(x_m) \ell_m,
    \]
    and so
    \[
        \int_{-1}^1 r(x) \, \d x
        = \sum_{i=0}^{m} r(x_i) \int_{-1}^1 \ell_i(x) \, \d x
        = \sum_{i=0}^{m} r(x_i) w_i,
    \]
    ...
    Next,
    we show that the nodes are necessarily the roots of $L_{m+1}$.
    To this end,
    assume that $x_0, \dotsc, x_m$ and weights $w_0, \dotsc, w_m$ are such that the equations~\eqref{eq:system_gauss_legendre} are satisfied,
    and let 
    \[
        q(x) = (x - x_0) \dotsc (x - x_m).
    \]
    Our goal is to show that $q(x)$ coincides with $L_{m+1}$ up to a constant factor.
    In order to show this, 
    it is sufficient to prove that $q(x)$ is orthogonal to $x^d$ for all $d = 0, \dotsc, m$,
    because the only polynomial in~$\poly(m+1)$ that satisfies these orthogonality relations is the Legendre polynomial~$L_{m+1}$,
    or a multiple thereof.
    For the values of~$i$ considered, the polynomial~$q(x) x^d$ belongs to $\poly(2n+1)$.
    Given that the integration rule with nodes $x_0, \dotsc, x_m$ and weights $w_0,\dotsc, w_m$ is exact for all the elements of $\poly(2n+1)$ by~\eqref{eq:system_gauss_legendre} by assumption,
    we deduce that
    \[
    ...
    Since integration rule must be exact for any Lagrange polynomial $\ell_j$,
    we have
    \[
        \int_{-1}^{1} \ell_j(x) \, \d x = \sum_{i=0}^{m} w_i \ell_j(x_i) = w_j,
    \]
