...
to any matrix $A \in \real^{m \times n}$, there naturally corresponds the linear operator from $\real^n$ to $\real^m$ with action~$\vect x \mapsto A \vect x$.
...
\begin{remark}
There is a definition parallel to \cref{definition:diagonalizable} for matrices in $A \in \complex^{n \times n}$.
...
the main one being that they are diagonalizable with a matrix $P$ that is orthogonal,
i.e. such that $P^{-1} = P^\t$.
