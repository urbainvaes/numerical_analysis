...
\begin{proof}
    ...
    that~$\norm{\vect x^{(i)}}_{\beta} \to 0$ as $i \to \infty$ but $\bigl\lvert \lambda_1^{(i)} \bigr\rvert + \dotsb + \bigl\lvert \lambda_n^{(i)} \bigr\rvert = 1$ for all $i \in \nat$.
    ...
    Since $\vect x^{(i)} \to \vect 0$ in this norm by assumption,
