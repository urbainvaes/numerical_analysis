...
\[
    \norm{\mat A \mat B} \leq \norm{\mat A} \norm{\mat B}.
\]
The $p$-norms are submultiplicative, and so is the Frobenius norm.
...
\begin{definition}
    ...
    and a diagonal matrix $\mat \Lambda \in \real^{n \times n}$ such that
    \begin{equation}
        \label{eq:eigen_decomposition}
        \mat A \mat P = \mat P \mat D.
    \end{equation}
    In this case,
    the diagonal elements of $\mat \Lambda$ are called the eigenvalues of~$\mat A$,
    and the columns of $\mat P$ are called the eigenvectors of~$\mat A$.
\end{definition}
Denoting by $\vect e_i$ the $i$-th column of $\mat P$ and by $\lambda_i$ the $i$-th diagonal element of $\mat \Lambda$,
...
\begin{theorem}
    ...
    then there exists an orthogonal matrix $\mat Q \in \real^{n \times n}$ and a diagonal matrix $\mat \Lambda \in \real^{n \times n}$ such that
    \[
        \mat A \mat Q = \mat Q \mat \Lambda.
    \]
\end{theorem}
\begin{proof}
    [Sketch of the proof]
    ...
    % \begin{itemize}
        ...
        % \item
            ...
            we deduce that $\mat V_{n-1} = \mat Q_{n-1} \mat \Lambda_{n-1} \mat Q_{n-1}^\t$
            for appropriate matrices $\mat Q_{n-1} \in \real^{n-1 \times n-1}$ and $\mat \Lambda_{n-1} \in \real^{n-1 \times n-1}$
            which are orthogonal and diagonal, respectively.

        % \item
            \vspace{.3cm}
            \textbf{Step 3. Constructing $\mat Q$ and $\mat \Lambda$}.
            Define now
            \[
                Q =
                V
...
\begin{exercise}
    ...
    then $\mat A^n = \mat P \mat \Lambda^n \mat P^{-1}$.
\end{exercise}
