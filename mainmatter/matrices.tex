...
The transpose of a matrix $\mat A \in \complex^{m \times n}$ is denoted by $\mat A^\t \in \real^{n \times m}$
...
we note that if $\vect e_1$ and $\vect e_2$ are eigenvectors of a Hermitian matrix associated with different eigenvalues,
...
\begin{proof}
    ...
    and prove that it then also holds for~$A \in \complex^{n \times n}$.
    % \begin{itemize}
        % \item
...
\begin{proof}
    ...
    we use the fact that $\mat N^{i}$ is a matrix with zeros everywhere except in the $i$-th super-diagonal,
...
\begin{proof}
    ...
    or any other matrix norm since they are all equivalent.
